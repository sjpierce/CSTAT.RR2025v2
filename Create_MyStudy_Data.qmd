---
title: "Create Example Data for Reproducible Research Presentation"
...
params:                               # Default values for parameters
  ...
  SourceFile: "Create_MyStudy_Data.qmd"  # Name of this script file
  LogFile: "Create_MyStudy_Data.html"    # Name of rendered output file
...
# Purpose
This script prepares simulated, fictional example data for my presentation 
[@Pierce-RN8764].
...
was stored when this file was rendered. For this chunk to work, you must have
the relevant parameters set properly in the YAML header.
...
library(piercer)        # for file_details(), git_report(), sim_2arm_prepost()
...
Setting the random number seed via `set.seed()` should make the dataset more 
reproducible across computers. If we don't do that, we'll get different data 
...
We use the `here` package's `here()` function to specify a file location
using a path name relative to relative to the project-level root directory 
identified above in the Declare Path section. 

Now we actually save out the data file. This is fictional data, so we can safely 
commit it to a public GitHub repository and track changes to it. For 
reproducibility purposes, I document the name, size, and last modified date for 
the data file after saving it. 
