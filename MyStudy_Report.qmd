...
params:                               # Default values for parameters
  SourceDir: ""                       # Relative path to folder holding this file
  SourceFile: "MyStudy_Report.qmd"    # Name of this script file
  LogFile: "MyStudy_Report.html"      # Name of rendered output file
...
execute:                              # Default Quarto chunk execution options
  ...
  output: true                        # Include the results of code in output? 
  warning: true                       # include warnings in output?
  error: true                         # Include error messages in output?
  include: true                       # Include both code & results in output? 
knitr:                                # Default R knitr package chunk options
  opts_chunk: 
    message: true                     # Include R messages produced by code? 
---

# Purpose
This report summarizes findings from analyses of the MyStudy data, which is 
simulated, fictional example data for my presentation [@Pierce-RN8764]. 
...
as expected. The message it produces also documents where the local repository
was stored when this file was rendered.
...
::: {.callout-tip}
For the chunk above to work, you must have the relevant parameters set properly
in the YAML header.
...
:::{.callout-tip}
The *piercer* package is available from my public GithHub repository at 
[https://github.com/sjpierce/piercer](https://github.com/sjpierce/piercer). 
:::
...
::: {.callout-important}
The data file is already stored in the `CSTAT.RR2025v2` repository, but you 
could also run the `Create_MyStudy_Data.qmd` script to re-create it. 
:::

To satisfy the **documentation** criterion for reproducibility, the next chunk 
reports the name, size, and last modified date for the data file after loading 
it. 
...
file_details(here("data/MyStudy.RData"))
```
...
each group (see @tbl-emm-Group) and visualize the result in @fig-emm-Group. The 
unstandardized effect size for the group effect is shown in 
...
emm_Group <- avg_predictions(m1, variables = "Group") %>% 
  as_tibble(.) %>% 
  rename(Mean = estimate, SE = std.error, LL= conf.low, UL= conf.high) %>% 
  mutate(Group = factor(Group, levels = c("Ctl", "Trt"), 
                        labels = c("Control", "Treatment"))) %>% 
  select(Group, Mean, SE, LL, UL)
...
emm_Group %>% 
  ...
  kable_styling(full_width = FALSE) %>% 
  add_header_above(header = c(" " = 3, "95% CI" = 2)) %>% 
  footnote(general = FN, general_title = "Note: ", footnote_as_chunk = TRUE) %>% 
  cat()
```
...
emm_Group %>% 
  ggplot(data = ., mapping = aes(x = Group, y = Mean)) + 
  geom_errorbar(aes(ymin = LL, ymax = UL), width = .1) + 
  geom_point(size = 2) + 
  ylab("Posttest Score") + 
  xlab("Group") + 
  scale_y_continuous(lim = c(70, 90)) +
  theme_bw()
```

```{r}
#| label:  tbl-emm-Group-difftest
#| tbl-cap: "Unstandardized Effect Size for Difference Between Group Means"
#| results: asis

# Table footnote.
FN <- "Estimates are in the units of the posttest scores."

avg_comparisons(m1, variables = list(Group = "pairwise"), 
                comparison = "difference") %>% 
  as_tibble() %>% 
  rename(Contrast = contrast, Estimate = estimate, SE = std.error, z = statistic, 
         p = p.value, LL = conf.low, UL = conf.high) %>% 
  select(Contrast, Estimate, SE, LL, UL, z, p) %>% 
...
::: {.callout-tip}
I used the [`marginaleffects`](https://cran.r-project.org/package=marginaleffects) 
